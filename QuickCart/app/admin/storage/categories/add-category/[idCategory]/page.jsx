'use client';
import React, { useEffect } from 'react';
import { useRouter, useParams } from 'next/navigation';
import { useCategoryForm } from '@/hooks/local/useCategoryForm';
import { useUpdateCategory, useCategory } from '@/hooks/server/useCategories';
import { CategoryForm } from '@/components/admin/category-form/page';
import toast from 'react-hot-toast';
import { useAuth } from "@/hooks/server/useAuth";

const EditCategoryPage = () => {
  const router = useRouter();
  const { idCategory } = useParams();
  const { token } = useAuth();
  const categoryForm = useCategoryForm();

  // Cargar datos de la categoría
  const { data: categoryData, isLoading } = useCategory(idCategory);

  useEffect(() => {
    if (idCategory && categoryData) {
      categoryForm.setFormData({
        nombre: categoryData.nombre || '',
        estado: categoryData.estado ?? true,
      });
    }
  }, [idCategory, categoryData]);

  const { mutate: updateCategory } = useUpdateCategory();

  const handleSubmit = async (e) => {
    e?.preventDefault();
    
    if (!categoryForm.validate()) {
      toast.error('Por favor completa todos los campos requeridos correctamente');
      return false;
    }

    categoryForm.setIsSubmitting(true);

    try {
      const payload = {
        nombre: categoryForm.formData.nombre,
        estado: categoryForm.formData.estado,
      };

      updateCategory(
        { id: idCategory, categoryData: payload, token },
        {
          onSuccess: (data) => {
            toast.success('Categoría actualizada exitosamente');
            categoryForm.resetForm();
            router.push('/admin/storage/categories');
          },
          onError: (error) => {
            const errorMessage = error?.response?.data?.message || 'Error al actualizar';
            toast.error(errorMessage);
          },
        }
      );

      return true;
    } catch (error) {
      const errorMessage = error?.response?.data?.message || error?.message || 'Error inesperado';
      toast.error(errorMessage);
      return false;
    } finally {
      categoryForm.setIsSubmitting(false);
    }
  };

  const handleCancel = () => {
    if (confirm('¿Seguro que deseas cancelar? Los cambios no guardados se perderán.')) {
      router.back();
    }
  };

  if (isLoading) {
    return (
      <div className="flex justify-center items-center min-h-screen">
        <div className="text-lg text-gray-600">Cargando categoría...</div>
      </div>
    );
  }

  if (!categoryData) {
    return (
      <div className="flex justify-center items-center min-h-screen">
        <div className="text-lg text-red-600">No se encontró la categoría</div>
      </div>
    );
  }

  return (
    <div className="flex-1 min-h-screen bg-gray-50">
      <div className="md:p-10 p-4 max-w-5xl mx-auto">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900">
            Editar Categoría
          </h1>
          <p className="text-gray-600 mt-1">
            Los campos marcados con (*) son obligatorios
          </p>
        </div>
        <CategoryForm
          {...categoryForm}
          onSubmit={handleSubmit}
          onCancel={handleCancel}
        />
      </div>
    </div>
  );
};

export default EditCategoryPage;